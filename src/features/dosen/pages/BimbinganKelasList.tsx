...
export const BimbinganKelasList: React.FC = () => {
  ...
  const [sessions, setSessions] = useState<ClassGuidanceSession[]>([]);
  const [selectedSession, setSelectedSession] = useState<ClassGuidanceSession | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const fetchedSessions = await dosenService.getClassSessions(assignmentIds);
      setSessions(fetchedSessions);

      if (fetchedSessions.length > 0) {
        const activeSess = selectedSession 
          ? fetchedSessions.find((s) => s.id === selectedSession.id) || fetchedSessions[0]
          : fetchedSessions[0];
        setSelectedSession(activeSess);

        const parts = await dosenService.getParticipants([activeSess.id]);
      ...
      if (asgs.length > 0) {
        setFormData((prev) => ({
          ...prev,
          assignment_id: prev.assignment_id || asgs[0].id
        }));
      }
    } catch (err) {
      console.error('Error loading BimbinganKelasList data:', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSelectSession = async (session: ClassGuidanceSession) => {
    setSelectedSession(session);
    try {
      const parts = await dosenService.getParticipants([session.id]);
      setParticipants(parts);
    } catch (err) {
      console.error('Error loading participants for session:', err);
    }
  };
  ...
  const handleCreateSession = async (e: React.FormEvent) => {
    ...
    try {
      const created = await dosenService.createClassSession({
      ...
      if (created) {
        ...
        const parts = await dosenService.getParticipants([created.id]);
        setParticipants(parts);
      ...
      alert(`Gagal membuat sesi bimbingan: ${err?.message || 'Terjadi kesalahan'}`);
    }
  };
  ...
  const handleConfirmValidation = async (e: React.FormEvent) => {
    ...
    try {
      await dosenService.validateParticipant(
        targetParticipant.id,
        lecturerId,
        feedbackInput.trim() || undefined
      );

      const parts = await dosenService.getParticipants([selectedSession.id]);
      setParticipants(parts);
      setIsValidateModalOpen(false);
      showToast(`✓ Bimbingan ${targetParticipant.student?.profile?.full_name} berhasil divalidasi. Paraf Dosen resmi telah aktif.`);
    } catch (err: any) {
      alert(`Gagal validasi: ${err?.message || 'Terjadi kesalahan'}`);
    }
  };

  const handleValidateAllHadir = async () => {
    ...
    try {
      await dosenService.validateAllSessionParticipants(selectedSession.id, lecturerId);
      const parts = await dosenService.getParticipants([selectedSession.id]);
      setParticipants(parts);
      showToast(`✓ ${hadirPending.length} mahasiswa berstatus HADIR berhasil divalidasi secara massal.`);
    } catch (err: any) {
      alert(`Gagal validasi massal: ${err?.message || 'Terjadi kesalahan'}`);
    }
  };
    ...
    return (
      <div className="p-6 sm:p-8 max-w-[1400px] mx-auto min-h-[400px] flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs font-semibold text-slate-500">Memuat sesi bimbingan kelas...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="space-y-2.5">
            ...
            {sessions.length === 0 ? (
              <div className="bg-white p-6 rounded-md border border-slate-200 text-center text-xs text-slate-500">
                Belum ada sesi bimbingan kelas. Silakan klik tombol "Buat Sesi Kelas Baru" untuk memulai.
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="px-2 py-0.5 rounded text-[10.5px] font-bold bg-slate-100 text-slate-800 border border-slate-200">
                        Kelas {session.assignment?.class?.name || 'PA'}
                      </span>
                    ...
                    <div className="mt-2.5 pt-2 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                      <span className="text-emerald-700 font-medium">
                        Pelaksanaan: {formatDate(session.session_date)}
                      </span>
          ...
          <div className="lg:col-span-2 space-y-4">
            {selectedSession ? (
                        ...
                        {participants.length === 0 ? (
                          <tr>
                            <td colSpan={6} className="py-6 text-center text-slate-400">
                              Tidak ada data peserta mahasiswa pada sesi ini.
                            </td>
                          </tr>
                        ) : (
              ...
              <div className="bg-white p-8 rounded-md border border-slate-200 text-center text-xs text-slate-500">
                Pilih sesi bimbingan kelas terlebih dahulu atau buat sesi baru.
              </div>
            )}
          </div>
        </div>
